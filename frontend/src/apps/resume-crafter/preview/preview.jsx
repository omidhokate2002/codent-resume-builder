...
export const Preview = () => {
  ...
  const resumeById = resumeContext?.resumeById || {};
  
  const fetchDataById = useMemo(() => {
    return resumeContext?.fetchDataById || (() => {
      console.warn('fetchDataById not available from context');
    });
  }, [resumeContext?.fetchDataById]);

  useEffect(() => {
    if (state && fetchDataById) {
      fetchDataById(state);
    }
  }, [state, fetchDataById]);
